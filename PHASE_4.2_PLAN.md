...
1. **Public programme shows Confirmed events only, with no status badge.** Approved / Proposed / Sent back stay off this page. Status labels come back on the committee calendar (separate job): that view shows all events, filters by status, and uses `law_calendar_status_badge()`.
