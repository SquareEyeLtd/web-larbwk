...
class BookingsDashboardTest extends LAW_Test_Case {
	...
	private function fixture(): array {
		...
		$booking_a   = $this->make_booking( $event_a, $owner_a, array( array( 'name' => 'Grace Guest', 'email' => $guest_email, 'organisation' => 'Guest LLP', 'job_title' => 'Partner' ) ) );

		$owner_b   = $this->make_user( 'attendee' );
		$booking_b = $this->make_booking( $event_b, $owner_b, array() );

		$owner_c   = $this->make_user( 'attendee' );
		$booking_c = $this->make_booking( $event_b, $owner_c, array() );
		...
		return compact( 'event_a', 'event_b', 'owner_a', 'guest_email', 'booking_a', 'owner_b', 'booking_b', 'owner_c', 'booking_c', 'press_email', 'booking_p' );
	}
	...
	public function test_rows_flatten_active_bookings_across_events_with_live_country(): void {
		...
		$mine = array_filter( $data['rows'], fn( $r ) => in_array( $r['booking_id'], array( $f['booking_a'], $f['booking_b'], $f['booking_p'] ), true ) );
		$this->assertCount( 4, $mine, 'Owner + guest on A, owner on B, press on A; the cancelled booking is excluded by default.' );
		...
		$this->assertTrue( $owner_row['is_owner'] );
	...
	public function test_export_rows_columns_and_press_marker(): void {
		...
		$this->assertSame(
			array( 'Booking ID', 'Event', 'Event date', 'Reference', 'First name', 'Second name', 'Email', 'Organisation', 'Job title', 'Country', 'Press', 'Status', 'Booked on', 'Accessibility', 'Dietary' ),
			$data['columns']
		);
		...
		$press = array_values( array_filter( $data['rows'], fn( $r ) => $f['press_email'] === $r[6] ) );
		$this->assertCount( 1, $press );
		$this->assertSame( 'Pat', $press[0][4] );
		$this->assertSame( 'Press', $press[0][5] );
		$this->assertSame( 'Yes', $press[0][10] );
		$this->assertSame( 'Active', $press[0][11] );

		$owner = array_values( array_filter( $data['rows'], fn( $r ) => get_userdata( $f['owner_a'] )->user_email === $r[6] ) );
		$this->assertSame( 'France', $owner[0][9] );
		$this->assertSame( '', $owner[0][10] );
	}
