 ...
 *
 * It posts law_booking_update_card, which dispatches by law_booking_kind(), so
 * a flagship application and a reception waitlist entry share one handler
 * (RECEPTIONS.md §1.5). The older law_flagship_update_card action stays
 * registered for any page already open in somebody's browser.
 *
...
<form class="law-booking-form law-flagship-card-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
	<input type="hidden" name="action" value="law_booking_update_card">
	<input type="hidden" name="booking_id" value="<?php echo esc_attr( (string) $law_fc_id ); ?>">
	<?php wp_nonce_field( 'law_booking_update_card' ); ?>
