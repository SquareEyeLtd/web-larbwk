...
<?php if ( is_user_logged_in() ) : ?>
	<?php
	// The post-booking dialog booking-form.js opens. Its only exits are the two
	// links (the script marks it law-modal--busy, so Escape and overlay clicks
	// are inert): closing in place would leave a stale "Book now" behind it.
	?>
	<div class="law-modal" id="law-booking-success" hidden>
		...
		<div class="law-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="law-booking-success-title" tabindex="-1">
			...
			<p class="law-modal__copy"><?php echo esc_html( sprintf( __( "You're booked onto %s. A confirmation email with a calendar invitation is on its way to you.", 'law' ), (string) $law_bk_event['title'] ) ); ?></p>
			...
			<p class="law-modal__actions">
				<a class="button second" href="<?php echo esc_url( $law_bk_permalink ); ?>"><?php esc_html_e( 'Close', 'law' ); ?></a>
