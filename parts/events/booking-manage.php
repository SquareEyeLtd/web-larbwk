...
if (function_exists('law_flagship_booking_is') && law_flagship_booking_is($law_bm_booking)) {
    get_template_part('parts/events/flagship-manage-application', null, [ 'booking_id' => $law_bm_id ]);
    return;
}
...
if (! $law_bm_party && $law_bm_cancelled) {
    $law_bm_party = [ $law_bm_booking ]; // Show the cancelled row rather than an empty page.
}
$law_bm_own = null;
...
get_template_part('parts/events/booking-payment-facts', null, [ 'booking_id' => $law_bm_id ]);
		    ?>
		...
		<ul class="law-dashboard__people law-booking-manage__people">
			<?php foreach ($law_bm_party as $law_bm_entry) :
			    ...
			    $law_bm_facts      = array_filter([ $law_bm_person['organisation'], $law_bm_person['job_title'] ]);
			    ...
			    if ($law_bm_row_wait && $law_bm_row_self) {
			        ...
			        $law_bm_copy = __('Your place is released for someone else. Your colleagues keep theirs and you can still manage their bookings here.', 'law');
			    } elseif ($law_bm_row_self) {
			        $law_bm_copy = __('Your place is released for someone else. You can book again later if places are still available.', 'law');
			    } else {
			        $law_bm_copy = sprintf(__('This cancels the booking for %s and releases their place. They are emailed to let them know.', 'law'), $law_bm_row_name);
			    }
			    ?>
				<li>
