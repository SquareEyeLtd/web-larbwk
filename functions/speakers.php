...
/**
 * The Speakers archive URL, for the settings screen and the profile back link.
 *
 * Always resolves from the page assigned templates/speakers.php, falling back
 * to /speakers/ when no page carries the template yet.
 */
function law_speakers_archive_url() {
	$page_id = law_speakers_page_id();
	return $page_id ? (string) get_permalink( $page_id ) : home_url( '/speakers/' );
}
