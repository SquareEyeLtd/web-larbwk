...
function law_events_map_post( $post, $allowed = null ) {
	...
	return array(
		...
		// The committee's "External" switch, behind the programme's Organiser
		...
		// external_url rides along because the card's Register button links
		// straight out to it, and a programme day renders up to 500 cards: one
		// meta read here beats one per card.
		'is_external'        => (bool) law_event_meta( $post->ID, '_law_is_external' ),
		'external_url'       => (string) law_event_meta( $post->ID, '_law_external_url' ),
